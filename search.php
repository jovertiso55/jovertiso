...
<tr>
<td>
<input style="width:65%" class="postmsg" type="text" placeholder="Search people here" name="msgtext"/>
...
?>
 ...
 <?php
include 'conn.php';
$sel = "SELECT concat (lname,', ',fname) as fullname,image
		FROM `user_tb` 
		WHERE 1 ORDER BY RecNo DESC";
...
while ($row = mysqli_fetch_assoc($adap))
{
	...
	echo "<td class='search_td'><img class='imgs' src='../images/users/". $row['image'] ."' alt='Avatar' style=width:30px;'>      ". $row['fullname'] ."</td>";	
	echo "</tr>";
	echo "<tr>";
	echo "<td><p></p></td>";	
	echo "</tr>";
