...
<tr>
<td>
<input style="width:65%" class="postmsg" type="text" placeholder="Search Events here" name="msgtext"/>
...
$sel = "SELECT * 
		...
		LEFT JOIN (SELECT recno,concat(fname,' ',lname) as fullname
				  FROM user_tb)t2
				  on t1.posted_recno = t2.recno
		WHERE status<>'closed'
...
while ($row = mysqli_fetch_assoc($adap))
{
	...
	echo "<tr>";
	echo "<td class='event_td'><h3>". $row['event_title'] ." <br/><a href='products.php' />View product(s)</h3></td>";	
	echo "</tr>";
	echo "<tr>";
	echo "<td class='event_td'>". $row['event_desc'] ."</td>";	
	echo "</tr>";
	echo "<tr>";
	// echo "<td class='event_td'><img src='../images/Full.jpg' style='width:100%'></td>";	
	echo "<td class='event_td'><img src='../images/". $row['image'] ."' style='width:100%'></td>";	
	echo "</tr>";
	echo "<tr>";
	echo "<td class='event_td'>Posted by: ". $row['fullname'] ."</td>";	
	echo "</tr>";
	echo "<tr>";
	echo "<td class='event_td'>Date posted: ". $row['posted_date'] ."</td>";	
	echo "</tr>";
	echo "<table/>";
	echo "<br/>";
